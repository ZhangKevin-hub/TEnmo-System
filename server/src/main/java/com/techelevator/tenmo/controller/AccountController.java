...
public class AccountController {
    ...
    public void createAccount(Principal currentUser) {
        ...
        accountDao.createAdditionalAccount(currentUserId);
    }

    @RequestMapping(path="/accounts/{accountId}/balance", method=RequestMethod.GET)
    public BigDecimal getBalance(Principal currentUser, @PathVariable @Valid int accountId) {
        currentUserId = userDao.findByUsername(currentUser.getName()).getId();
        ...
        BigDecimal balance = accountDao.getBalance(currentUserId,accountId);
    ...
    public Transfer instantTransfer(@Valid @RequestBody Transfer transfer, Principal currentUser) {
        ...
        if (accountDao.getBalance(currentUserId,sendingAccount.getAccount_id()).compareTo(transfer.getTransfer_amount()) != -1) {
            ...
            if (currentUserId == sendingAccount.getUserId()) {
                ...
                if (sendingAccount.getAccount_id() != receivingAccount.getAccount_id()) {
                    // Check if the user associated with the receiving account exists in the List of accounts that exclude sending Account
                    if (accountDao.getAccountList(sendingAccount.getAccount_id()).contains(receivingAccount.getAccount_id())) {
    ...
    public String createRequest(@Valid @RequestBody Transfer transfer, Principal principal) {
        ...
        if (currentUserId == receivingAccount.getUserId()) {
            ...
            if (sendingAccount.getUserId() != receivingAccount.getUserId()) {
                ...
                // See if receiving account is present in list of accounts that excludes the receiving account id
                if (accountDao.getAccountList(receivingAccount.getAccount_id()).contains(sendingAccount)) {
    ...
    public String rejectRequest(@Valid @PathVariable int id, Principal currentUser) {
        ...
        if (transferDao.getTransferByTransferId(id, currentUserId) != null) {
            // Check if the transfer is a pending request
            if (transfer.getTransfer_status().equals("Pending")) {
                // Check if User is the sending Account
                if(currentUserId == sendingAccount.getUserId()) {
                     // Reject the request
